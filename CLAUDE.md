...
- `.claude/rules/design-redirect-ui.md` — Manual Redirect creation feature (tracked in [#364](https://github.com/gazetta-studio/gazetta-studio/issues/364)). 8 locked decisions: Redirect as first-class CONTEXT.md noun, new `POST /api/redirects` endpoint, SiteTree "+ New redirect" entry, hard-refuse on live-name collision, both pages+fragments, new `create-redirect` audit action, `edit:pages`/`edit:fragments` capability.
- `.claude/rules/design-redirect-ui-implementation.md` — 7-cut sequence (~6 days). Schema refinement, audit enum, route + schemas, dialog + tree button, conflict UX, capability-gap composition, e2e + docs.
