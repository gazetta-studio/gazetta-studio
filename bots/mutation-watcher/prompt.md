...
`CLOSED_FOLLOWUP_WINDOW_DAYS` is the recency threshold for the
closed-prior-issue suppression check (default 30). See "Match
found, issue is CLOSED RECENTLY" below.
...
- **Match found, issue is CLOSED RECENTLY** (within
  `CLOSED_FOLLOWUP_WINDOW_DAYS`, default 30 days) → skip entirely.
  Don't file, don't comment. A prior fix attempt landed recently;
  trust the human to re-open or file a fresh issue while the context
  is still warm. The bot's job is to surface NEW signal, not pile
  onto in-flight work.
- **Match found, issue is CLOSED OLD** (closed more than
  `CLOSED_FOLLOWUP_WINDOW_DAYS` ago) → file a new issue per step 3b,
  but **reference the prior issue** in the body ("prior fix attempt:
  closed #NNN on YYYY-MM-DD"). The old closure means the prior fix
  cycle has had time to land; if mutants are STILL surviving, that's
  new signal worth tracking separately. Don't auto-link "Fixes" —
  link via a "Background" section so the human can decide whether
  to re-open the old issue or close it as won't-fix.
- **No match** → file a new issue per step 3b.

To check the closed date:

```
gh issue view <NUMBER> --json closedAt
```

The threshold is the env-overridable `CLOSED_FOLLOWUP_WINDOW_DAYS`,
defaulted to 30 in the workflow. If the env var is not set, fall
back to 30 days.

Document the skip-or-file decision with a `> Decision: ...` line so
the transcript records why nothing was filed (or why a fresh issue
was filed despite a prior closed one).
...
### 3b. No match (or only stale closed match) → file a new issue
...
<if filing despite a closed prior issue from beyond the
CLOSED_FOLLOWUP_WINDOW_DAYS window, include this section first:

## Background

A prior fix attempt landed and closed #PRIOR on YYYY-MM-DD. That
closure is older than CLOSED_FOLLOWUP_WINDOW_DAYS days, so this is
a fresh-cycle re-file: surviving mutants in this file persist
despite the prior attempt. Decide whether to re-open #PRIOR with a
new commit OR close this as won't-fix; do NOT auto-link with
"Fixes" — the original cycle is past and the right resolution path
depends on what's changed since.
>
