...
> **Partial supersession by [ADR-0008](0008-provider-factory-returns-instance.md):** the operator-facing config-shape consequence ("every Provider follows `provider: 'name'`") is superseded — operators write factory calls returning Provider instances, not literal `{ provider: 'name', ...config }` blocks. The eight Universal Provider Requirements (multi-instance correctness, env-var credentials, fail-mode declared, never-throw-on-transport-errors at recording layer, stable typed contract, independent error taxonomy, sensible defaults, stateless interface) describe Provider INTERNALS and remain in force unchanged.
