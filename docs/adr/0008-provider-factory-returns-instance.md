...
Three narrow data-layer exceptions are accepted:
- **Exception A**: `gazetta.config.ts defaults.cache` accepts raw options (not a constructed instance) so each inheriting site builds its own per-site instance — required by `design-cache.md`'s per-site invariant.
- **Exception B**: AI task config (model, systemPrompt, maxTokens) is a data-literal across three rungs (gazetta → site → target). Per-target `altText.ai` sub-block accepts `provider` (factory-call), `model`, `systemPrompt`, `maxTokens` (data literals). Justified structurally: AI is the only Pattern-A surface that multiplexes one transport across multiple distinct request shapes (alt-text, translation, summarization, image-gen) with operator-tunable per-task config. Three rungs because each carries documented operator value (gazetta-level for agency multi-site setups; site-level for per-site brand voice; target-level for environment-specific tuning). See `design-provider-config.md` "Why AI is the outlier" for the structural test that determines Exception B eligibility — Search would qualify if shipped; Storage/Transform/Cache/Audit/Auth/Notification/Deploy don't.
- **Exception C**: per-target behavior-only overrides like `targets.X.altText: { auto: false, maxImageEdge: 1024 }` are partial literals for runtime knobs the suggester reads per call — semantically distinct from AI task config because they don't flow into adapter construction.
