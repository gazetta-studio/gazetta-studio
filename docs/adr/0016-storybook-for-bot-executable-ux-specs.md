...
**Decision: push the screenshots to the PR's feature branch and embed them inline via `raw.githubusercontent.com/{repo}/{branch}/…` URLs** — the standard CI-native, token-only mechanism (the default mode of `opengisch/comment-pr-with-images`). Works headless with `GH_TOKEN`; produces true inline images during review.

**Accepted trade-off — ephemeral by design:** the raw URLs 404 once the feature branch is auto-deleted on merge, and the PNGs live transiently on that branch (never reaching `main`, since the branch is squashed/deleted). This is acceptable because the use case is *during-review* visibility — reviewers want the images while the PR is open, exactly when the branch exists. Screenshots are an **ephemeral review aid, not a durable artifact.** Chosen over a dedicated long-lived `screenshots` branch (persistent but accumulates binaries + needs a prune job) and external hosting (Imgur/S3 — extra credentials + a third-party dependency).

**Best-effort + mandatory fallback:** the commit-and-push is best-effort and **must never block the PR.** On any failure (push rejected, screenshot capture errored), feature-bot opens the PR *without* the screenshots plus a one-line note (`_(story screenshots unavailable — run `npm run storybook` on the branch)_`) and logs it. Images are an enhancement, never a dependency.
...
| Host | Inline image? | Token-only headless? | Persists past merge | Main-repo bloat | Verdict |
|---|---|---|---|---|---|
| Paste / comment / discussion asset (`user-attachments`) | ✅ | ❌ — asset pipeline is browser-cookie-only, not token ([community #46951](https://github.com/orgs/community/discussions/46951), [cli/cli#13256](https://github.com/cli/cli/issues/13256)) | ✅ | none | **Impossible for a token bot** |
| Actions artifact | ❌ — auth'd download page, not raw bytes ([upload-artifact](https://github.com/actions/upload-artifact)) | ✅ | ⚠️ 90d | none | Rejected (not inline) |
| **Feature-branch commit + `raw.githubusercontent` (chosen)** | ✅ | ✅ | ❌ 404 on merge | transient, on the deleted branch only | **Chosen** — simplest; ephemeral matches during-review use case; zero cleanup |
| Dedicated `screenshots` branch | ✅ | ✅ | ✅ | none (separate branch) | Rejected — accumulates binaries + needs a prune job |
| Release asset (`gh release upload`) | ✅ | ✅ | ✅ | none | Viable but rejected — clutters Releases tab + needs retention; semantic smell (release ≠ review screenshot) |
| Wiki repo (`raw.githubusercontent.com/wiki/…`) | ✅ | ✅ | ✅ | none (separate `.wiki.git`) | Viable but rejected — needs wiki enabled + obscure mechanism |

Feature-branch commit was chosen over the persistent options (release, wiki) precisely *because* the screenshots are an ephemeral review aid — persistence buys nothing for during-review visibility, and the persistent options each carry a cleanup/clutter cost the ephemeral one avoids by dying with the branch.
