...
| 2 | History restore of an archived page → preserves archive marker, alias, and `archivedAt` fields | Restore writes manifest back to content tree; if archive fields are dropped by the restorer's whitelist, restored "live" pages still serve archived markers (or vice versa) | ☐ |
