...
- **Sidecar pattern** — Two roles ship: per-item sidecars and reverse-dep indices. Per-item sidecars carry `.{8hex}.hash` (content hash; powers `compareTargets`) and `.pub-{ts}` (publish timestamp + noindex flag), defined in [`sidecars.ts`](../../packages/gazetta/src/sidecars.ts). Reverse-dep relationships live in `.gazetta/{relation}/{target}/{source}` per-edge sidecars: `fragment-deps` (which pages/fragments reference each fragment) and `asset-refs` (which manifests reference each asset), built on the generic `DepRelation` abstraction in [`dep-sidecars.ts`](../../packages/gazetta/src/dep-sidecars.ts). Per-edge granularity gives multi-instance write correctness without optimistic concurrency. In-flight memoization on the read side (`/api/dependents` route's `fragmentDepsBackfill`) shares concurrent walks across callers. _Historical: the deleted `SourceSidecarWriter`/`source-sidecars.ts` and the `.uses-*`/`.tpl-*` forward sidecars were replaced by the per-edge reverse-dep indices in commit 7d61ce5 (Media v1 step 13c)._
