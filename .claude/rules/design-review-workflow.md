...
| 20 | **Agent B visual self-check + PR screenshot embed** — `bots/_lib` screenshot helper (Playwright, reuse `tools/mcp-dev` logic) + `build-storybook` step in `feature-bot.yml` + reviewer-prompt contract (screenshot each new/changed story → visual verdict vs design-doc state table → factor into APPROVE/REJECT) + **push the screenshots to the PR's feature branch and embed them inline via `raw.githubusercontent` URLs (best-effort; falls back to opening the PR without images on failure)** | 10 | agent | (bot infra) | Medium |
...
- Cut 20 adds **Agent B visual self-check + PR screenshot embed** (per [ADR-0016](../../docs/adr/0016-storybook-for-bot-executable-ux-specs.md) "validation stack" + "Screenshots embedded in the PR"): the reviewer screenshots each new/changed story, judges it against the design-doc state table (factoring into APPROVE/REJECT), **and embeds those screenshots in the PR body so the human reviewer sees rendered states inline.** The self-check catches *gross* visual failures (invisible element, overflow, collapsed layout) — advisory bot-loop, **not** a CI gate and **not** pixel-regression (LLM-judges-prose, no baseline; the real visual gate stays human PR review). The PR embed **pushes the screenshots to the PR's feature branch and embeds them inline via `raw.githubusercontent` URLs** — the standard token-only CI-native mechanism (per [ADR-0016](../../docs/adr/0016-storybook-for-bot-executable-ux-specs.md): paste-style `user-attachments` needs a browser session a headless token-only bot lacks; artifacts are link-only). Ephemeral by design — links 404 after the branch is auto-deleted on merge, which is fine because the use case is *during-review* visibility. Best-effort with a mandatory fallback: on failure the PR opens without images + a note, never blocked. UX cuts 11 + 14 depend on it. Depends only on Cut 10 (needs `build-storybook` working).
