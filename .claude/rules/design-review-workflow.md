...
| 20 | **Agent B visual self-check + PR screenshot embed** — `bots/_lib` screenshot helper (Playwright, reuse `tools/mcp-dev` logic) + `build-storybook` step in `feature-bot.yml` + reviewer-prompt contract (screenshot each new/changed story → visual verdict vs design-doc state table → factor into APPROVE/REJECT) + **embed the screenshots in the PR body via browser-flow upload to `user-attachments` (best-effort; falls back to opening the PR without images on upload failure)** | 10 | agent | (bot infra) | Medium |
