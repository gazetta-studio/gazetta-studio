...
- Cut 20 adds **Agent B visual self-check + PR screenshot embed** (per [ADR-0016](../../docs/adr/0016-storybook-for-bot-executable-ux-specs.md) "validation stack" + "Screenshots embedded in the PR"): the reviewer screenshots each new/changed story, judges it against the design-doc state table (factoring into APPROVE/REJECT), **and embeds those screenshots in the PR body so the human reviewer sees rendered states inline.** The self-check catches *gross* visual failures (invisible element, overflow, collapsed layout) — advisory bot-loop, **not** a CI gate and **not** pixel-regression (LLM-judges-prose, no baseline; the real visual gate stays human PR review). The PR embed **uploads the screenshots as GitHub Release assets** (`gh release upload`, rolling `bot-screenshots` tag) and embeds them inline via the public `…/releases/download/…` URLs (per [ADR-0016](../../docs/adr/0016-storybook-for-bot-executable-ux-specs.md): token-only, official API, public-served raw bytes that render inline). **The bytes never touch `main`** — which is *why* this beats the feature-branch-commit option (whose commits would merge into `main`'s history under squash/rebase/merge even though the branch is deleted). **Deterministic cleanup:** the bot deletes the cut's assets on PR merge/close (`gh release delete-asset`), with a scheduled reconciler pruning orphans as backstop. Best-effort with a mandatory fallback: on upload failure the PR opens without images + a note, never blocked; a cleanup failure never blocks the merge. UX cuts 11 + 14 depend on it. Depends only on Cut 10 (needs `build-storybook` working).
