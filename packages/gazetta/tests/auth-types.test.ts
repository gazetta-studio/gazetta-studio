...
describe('BUILT_IN_ROLES', () => {
  ...
  it('the five built-in role names are stable', () => {
    // Pin the names so consumers can rely on them; renaming any of
    // these is an audit-log + role-mapping breaking change.
    // `reviewer` + `publisher` land with the review-workflow Cut 2
    // (#516) — minimal additive roles aliasing the review/publish-
